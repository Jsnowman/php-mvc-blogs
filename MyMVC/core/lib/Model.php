...
abstract class Model
{
	...
	function __construct()
	{
		...
		//连接数据库，将成功连接后的资源保存起来 使用单例值连接一次数据库
		$this->link = \core\lib\MysqlLink::getMysqlLink();
		//得到数据表名函数
	...
	protected function getTableName()
	{
		...
		//获取类名，从类名中将表名得到
		$className = trim(strrchr(get_class($this),'\\'),'\\');
	...
	{
		//拼接缓存文件路径
		$cacheFile = './DBcache/'.$this->tableName.'.php';
    ...
    /**
     * @param $sql
     * @param bool $isdb 是否是查询数据表结构 如果为true 则为查询表结构
     * @return array|bool
     */
	function query($sql,$isdb = false)
	{
		//在这将options数组还原为初始状态
        if(!$isdb){
            $this->initOptions();
